...
public class Parser {
	 ...
	 * ErrorReporter from CompilerEnvirons.)
	 */
	public ScriptOrFnNode parse(Reader sourceReader, String sourceURI,	//函数返回一棵语法树
			int lineno) throws IOException {
		this.sourceURI = sourceURI;
		this.ts = new TokenStream(this, sourceReader, null, lineno);		//此处可能是词法分析
		return parse();
	}

	private ScriptOrFnNode parse() throws IOException {	//脚本或函数节点
		this.decompiler = createDecompiler(compilerEnv);	//创建反编译器
		this.nf = new IRFactory(this);
		currentScriptOrFn = nf.createScript();
		currentScope = currentScriptOrFn;
		int sourceStartOffset = decompiler.getCurrentOffset();	//当前位移量
		...
		int baseLineno = ts.getLineno(); // line number where source starts，从TokenStream（符号流）中找出源码的开始处

		/*
		 * so we have something to add nodes to until we've collected all the
		 * source
		 */
		Node pn = nf.createLeaf(Token.BLOCK);	//创建叶子节点
		...
		try {
			for (;;) {
				...
				nf.addChildToBack(pn, n);	//在pn后面加入孩子节点
			}
		} catch (StackOverflowError ex) {	//栈溢出异常
		...
		if (this.syntaxErrorCount != 0) {	//出现语法错误
		...
		nf.initScript(currentScriptOrFn, pn);	//初始化脚本
	...
	void defineSymbol(int declType, boolean ignoreNotInBlock, String name) {
		...
		if (symbol != null
			...
			case Token.LET:
				...
				if (!ignoreNotInBlock
						&& ((currentScopeType == Token.LOOP) || (currentScopeType == Token.IF))) {
	...
	private Node arrayComprehension(String arrayName, Node expr)
		...
		if (tt == Token.FOR) {
			...
			Node call = nf.createCallOrNew(Token.CALL, nf.createPropertyGet(
					nf.createName(arrayName), null, "push", 0));
